...
namespace sg {
	...
	class ComponentPropertyMetaTypeDelegate : public QItemDelegate {
	...
	class ComponentPropertyMetaModel : public QSqlQueryModel {
	...
	public:
		...
		ComponentPropertyMetaModel(Controller& controller, QObject* parent, QVariant component_id)
		: QSqlQueryModel(parent)
		, mController(controller)
		, mComponentId(std::move(component_id))
		{}

		~ComponentPropertyMetaModel() {
		}
		...
		bool setData(const QModelIndex& index, const QVariant& value, int role = Qt::EditRole) {
			...
			auto perform = [&](const QString& column_name, bool unique_check) -> Result<> {
				...
				if (unique_check && containsName(new_value)) {
					return Error(ComponentPropertyMetaEditor::tr("Property '%1' '%2' already exists").arg(column_name).arg(new_value));
				}
			...
			switch (index.column()) {
				...
				case NAME_COL: {
					...
					if (res.failed()) {
						MessageBoxCritical(ComponentPropertyMetaEditor::tr("Unable to rename property"), res.errorMessage(), res.errorInfo());
						return false;
					}
				}
				break;
				...
				case TYPE_COL: {
					...
					if (res.failed()) {
						MessageBoxCritical(ComponentPropertyMetaEditor::tr("Unable to change type"), res.errorMessage(), res.errorInfo());
						return false;
					}
				}
				break;
				...
				case DEFAULT_VALUE_COL: {
					...
					if (res.failed()) {
						MessageBoxCritical(ComponentPropertyMetaEditor::tr("Unable to change default value"), res.errorMessage(), res.errorInfo());
						return false;
					}
				}
				break;
	...
	ComponentPropertyMetaEditor::ComponentPropertyMetaEditor(Controller& controller, QVariant component_id, QWidget* parent)
	: QWidget(parent) {
		...
		auto model = new ComponentPropertyMetaModel(controller, this, component_id);
		...
		connect(add_button, &QPushButton::clicked, [component_id, &controller, model](){
			...
			auto res = perform();
			if (res.failed()) {
				MessageBoxCritical(ComponentPropertyMetaEditor::tr("Error creating component property"), res.errorMessage(), res.errorInfo());
			}
		});
		...
		view->setItemDelegateForColumn(1, new ComponentPropertyMetaTypeDelegate());
		layout->addWidget(view);

		auto proxy_model = new QSortFilterProxyModel(this);
		proxy_model->setSourceModel(model);
		proxy_model->setDynamicSortFilter(true);
		proxy_model->sort(ComponentPropertyMetaModel::NAME_COL);
		proxy_model->setFilterKeyColumn(ComponentPropertyMetaModel::NAME_COL);
		proxy_model->setFilterCaseSensitivity(Qt::CaseInsensitive);
		view->setModel(proxy_model);

		model->refresh();
		view->hideColumn(ComponentPropertyMetaModel::ID_COL);
		...
		connect(delete_row, &QAction::triggered, [view, model, proxy_model, &controller](bool){
			...
			auto perform = [&]() -> Result<> {
				...
				for (auto index : view->selectionModel()->selectedRows()) {
					QString id = proxy_model->data(proxy_model->index(index.row(), ComponentPropertyMetaModel::ID_COL)).toString();
	...
	ComponentPropertyMetaEditor::~ComponentPropertyMetaEditor() {
	}
}