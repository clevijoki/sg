...
namespace sg {
		...
		bool setData(const QModelIndex& index, const QVariant& value, int role = Qt::EditRole) {
			...
			auto perform = [&]() -> Result<> {
				...
				if (containsName(new_name)) {
					return Error(EntityEditor::tr("Entity named '%1' already exists").arg(new_name));
				}
			...
			auto res = perform();
			if (res.failed()) {
				MessageBoxCritical(EntityEditor::tr("Unable to rename entity"), res.errorMessage(), res.errorInfo());
				return false;
			}
	...
	EntityEditor::EntityEditor(Controller& controller, QWidget* parent) 
	: QWidget(parent) {
		...
		connect(new_button, &QPushButton::clicked, [view, model, &controller](){
			...
			auto res = perform();
			if (res.failed()) {
				MessageBoxCritical(EntityEditor::tr("Error creating entity"), res.errorMessage(), res.errorInfo());
			}
		});
